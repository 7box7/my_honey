...
> Используем комагду sleep 1000 &, чтобы создать пустой спящий процесс

> ps aux | grep sleep чтобы узнать PID процесса, запустим эту команду, выводящую информацию о всех процесс ps aux и найдем в них слово sleep при помощи grep, чтобы точно найти наш пустой процесс
...
┌──(kali㉿kali)-[~/Desktop]
└─$ ps aux | grep sleep
kali    64837   0.0  0.0 33591680     68 s014  R+    7:22PM   0:00.00 grep sleep
kali    64817   0.0  0.0 33596872    448 s014  S     7:22PM   0:00.00 sleep 1000
...
┌──(kali㉿kali)-[~/Desktop]
└─$ ps aux | grep sleep
kali    64949   0.0  0.0 33597012    588 s014  S+    7:25PM   0:00.00 grep sleep
...
┌──(kali㉿kali)-[~/Desktop]
└─$ ps aux | grep sleep
kali    65021   0.0  0.0 33597012    588 s014  S+    7:26PM   0:00.00 grep sleep
kali    65006   0.0  0.0 33597896    460 s014  S     7:26PM   0:00.00 sleep 1000

┌──(kali㉿kali)-[~/Desktop]
└─$ kill -9 65006
[1]+  Killed: 9               sleep 1000
...
┌──(kali㉿kali)-[~/Desktop]
└─$ ps aux | grep sleep
kali    65052   0.0  0.0 33597012    588 s014  S+    7:27PM   0:00.00 grep sleep
kali    65043   0.0  0.0 33596872    448 s014  S     7:26PM   0:00.00 sleep 1000
...
┌──(kali㉿kali)-[~/Desktop]
└─$ ps aux | grep sleep
kali    65092   0.0  0.0 33597012    588 s014  S+    7:27PM   0:00.00 grep sleep
kali    65043   0.0  0.0 33596872    448 s014  S     7:26PM   0:00.00 sleep 1000
...
┌──(kali㉿kali)-[~/Desktop]
└─$ ps aux | grep sleep
kali    65147   0.0  0.0 33597012    588 s014  S+    7:28PM   0:00.00 grep sleep
kali    65043   0.0  0.0 33596872    448 s014  S     7:26PM   0:00.00 sleep 1000
...
┌──(kali㉿kali)-[~/Desktop]
└─$ ps aux | grep sleep
kali    65260   0.0  0.0 33597012    588 s014  S+    7:30PM   0:00.00 grep sleep
kali    65043   0.0  0.0 33596872    448 s014  S     7:26PM   0:00.00 sleep 1000
```

Структура команды kill:

```
kill -[signal] [PID] (PID - Process ID)
```

* -SIGTERM или -15 пытается завершить процесс, если это возможно
* -SIGKILL или -9 принудительно завершает процесс
* -SIGSTOP или -18 останавливает процесс
* -SIGCONT или -19 возобновляет остановленный процесс
...
```
┌──(kali㉿kali)-[~/Desktop]
└─$ date +"%Y-%m-%d" | xargs -I {} sh -c 'ps -u $USER -o user,pid,start,comm | grep $USER > {}.log'
```

> В данной команде сначала при помощи команды date и указав формат +"%Y-%m-%d" мы получаем дату вида ГГГГ-ММ-ДД

> Дальше полученный вывод отправляется на ввод при помощи | в следующие команды

> xargs -I {} данная команда делает из ввода аргумент с названием {} для последующего выполнения команд

> Используем команду sh -c чтобы сделать следующую команду как единую структуру, а не как две команды (для корректной передачи параметра {}, иначе параметр передается в команду ps, а в следующей команде grep уже неизвестно что такое {})

> Дальше выполняется ps -u $USER -o user,pid,start,comm | grep $USER > {}.log, ps с флагом -u ищет все процессы текущего юзера, то есть kali, флаг -o делай вывод команды более емким, как мы хотим, в данном случае user,pid,start,comm. Далее вывод команды передается при помощи | в следующую команду grep, которая ищет во входных строках юзера kali, а затем при помощи > переводит вывод в файл с названием {}.log, где {} - текущая дата 

Часть файла 2024-12-06.log:

```
kali 53713 22Nov24 /Users/kali/.nvm/versions/node/v23.2.0/bin/node
kali  9599 16Nov24 /bin/bash
kali 11122 16Nov24 node
kali 21930 17Nov24 /bin/bash
kali 68378  7:56PM xargs
kali 68385  7:56PM sh
kali 68387  7:56PM grep
```
